...
DEPOSIT_FAUCET=5
# TOON-client pod (story 49.3) — persistent lease, slightly heavier
# than the faucet (anon daemon + viem + Fastify). ~$3-5/mo. Min deposit.
DEPOSIT_TOON_CLIENT=5
...
# toon-client SHA covers the Dockerfile + entrypoint source + schema
# contract + workspace pieces the entrypoint actually imports. Story 49.3.
TOON_CLIENT_SHA="$(
  for _sha_f in \
    "$ROOT/docker/Dockerfile.toon-client" \
    "$ROOT/docker/src/entrypoint-toon-client.ts" \
    "$ROOT/packages/townhouse/contracts/foreign-publish.schema.json" \
    "$ROOT/docker/townhouse-ator-sidecar/checksums.txt" \
  ; do
    if [ ! -f "$_sha_f" ]; then
      echo "[sha] ERROR: required file missing for TOON_CLIENT_SHA: $_sha_f" >&2
      exit 1
    fi
    cat "$_sha_f"
...
# TOON-client pod image (Story 49.3 — persistent Akash toon-client
# with POST /publish). Hosts the Fastify control plane + in-pod anon daemon.
TOON_CLIENT_IMAGE_TAGGED="ghcr.io/toon-protocol/toon-client:sha-$TOON_CLIENT_SHA"
TOON_CLIENT_IMAGE_DEMO="ghcr.io/toon-protocol/toon-client:demo"
...
# Build (and push) the TOON-Client pod image (Story 49.3).
...
# entrypoint-toon-client.ts.
cmd_build_toon_client() {
  require_cli docker
  echo "Building $TOON_CLIENT_IMAGE_TAGGED + :demo..."
  docker build \
    -f "$ROOT/docker/Dockerfile.toon-client" \
    -t "$TOON_CLIENT_IMAGE_TAGGED" \
    -t "$TOON_CLIENT_IMAGE_DEMO" \
    "$ROOT"
  echo "Pushing $TOON_CLIENT_IMAGE_TAGGED (SHA-pinned)..."
  docker push "$TOON_CLIENT_IMAGE_TAGGED" || {
    echo "[toon-client] ERROR: push of SHA-pinned tag failed." >&2
    echo "  Create the package first: https://github.com/orgs/toon-protocol/packages" >&2
    echo "  Then re-run: $0 build-toon-client" >&2
    exit 1
  }
  echo "Pushing $TOON_CLIENT_IMAGE_DEMO (canonical)..."
  docker push "$TOON_CLIENT_IMAGE_DEMO" || {
    echo "[toon-client] ERROR: push of $TOON_CLIENT_IMAGE_DEMO failed." >&2
    exit 1
  }
}

# Render the toon-client SDL (story 49.3). Reads FAUCET_URL +
...
render_toon_client_sdl() {
  local sdl_template="$SDL_DIR/toon-client.sdl.yaml"
  ...
  if [ -n "${TOON_FEE_PER_EVENT:-}" ]; then
    if ! [[ "${TOON_FEE_PER_EVENT}" =~ ^[0-9]+$ ]]; then
      echo "[toon-client] TOON_FEE_PER_EVENT must be a non-negative integer, got: ${TOON_FEE_PER_EVENT}" >&2
      exit 1
    fi
    ...
    if [ "$env_lines_before" -lt 1 ] || [ "$env_lines_after" -lt 1 ]; then
      echo "[toon-client] TOON_FEE_PER_EVENT override did not substitute (env_lines before=$env_lines_before, after=$env_lines_after with value=${TOON_FEE_PER_EVENT}) — SDL formatting drift?" >&2
      exit 1
    fi
    echo "[toon-client] Using TOON_FEE_PER_EVENT=${TOON_FEE_PER_EVENT} (override) — $env_lines_after env line(s) substituted" >&2
  fi
  echo "$rendered"
}
...
cmd_toon_client() {
  ...
  # toon-client pod depends on the faucet + chain leases being up. Warn loudly
  ...
  if [ -z "$faucet_url" ]; then
    echo "[toon-client] WARNING: faucet lease not found in $LEASES_FILE." >&2
    echo "[toon-client] Pod boot will fail at the faucet auto-fund step." >&2
    echo "[toon-client] Run: $0 build-faucet && $0 faucet" >&2
  fi
  if [ -z "$anvil_url" ] || [ -z "$solana_url" ]; then
    echo "[toon-client] WARNING: anvil/solana leases not both found." >&2
    echo "[toon-client] Pod will fail balance polling." >&2
  fi

  local rendered_sdl
  rendered_sdl="$(mktemp --suffix=.sdl.yaml)"
  trap 'rm -f "${rendered_sdl-}"' EXIT
  render_toon_client_sdl > "$rendered_sdl"

  local digest
  digest="$(image_digest "$TOON_CLIENT_IMAGE_DEMO")"
  ...
  deploy_sdl toon-client "$rendered_sdl" toon-client 8080 probe_toon_client_healthz "$DEPOSIT_TOON_CLIENT" "$digest"

  echo
  echo "[toon-client] Deployed."
  echo "  URL:        $(jq -r '."toon-client".url // "(pending)"' "$LEASES_FILE")"
  echo "  /healthz:   $(jq -r '."toon-client".url // "(pending)"' "$LEASES_FILE")/healthz"
  echo "  /publish:   $(jq -r '."toon-client".url // "(pending)"' "$LEASES_FILE")/publish"
  echo
  echo "  Lease owner: dev.jonathan.green@gmail.com"
  echo "  Sunset:      Close via \`$0 close toon-client\` when Epic 49 retires"
...
probe_toon_client_healthz() {
  local body
  body="$(curl -sf -k -m 8 --connect-timeout 5 "$1/healthz" 2>/dev/null || echo '')"
  echo "$body" | grep -q '"anyoneReady":[[:space:]]*true'
}

# Manual readiness probe for the toon-client lease (D-49.4-PR1-3).
# Reads the URL from leases.json and calls probe_toon_client_healthz which
# hits /healthz — bare / returns 404 from Fastify and is not a valid probe.
cmd_probe_toon_client() {
  ensure_leases_file
  local url
  url="$(jq -r '."toon-client".url // empty' "$LEASES_FILE")"
  if [ -z "$url" ]; then
    echo "ERROR: no toon-client lease in $LEASES_FILE" >&2
    exit 1
  fi
  echo "[probe-toon-client] Probing $url/healthz ..."
  local body
  body="$(curl -sf -k -m 10 --connect-timeout 5 "$url/healthz" 2>/dev/null || echo '')"
  if echo "$body" | grep -q '"anyoneReady":[[:space:]]*true'; then
    echo "[probe-toon-client] PASS — anyoneReady=true"
    echo "$body" | jq -c '{anyoneReady, evmAddr, solAddr, balances}' 2>/dev/null || echo "$body"
  else
    echo "[probe-toon-client] FAIL — anyoneReady not true (or pod unreachable)"
...
case "${1:-}" in
  ...
  build-toon-client) cmd_build_toon_client ;;
  anvil) cmd_anvil ;;
  solana) cmd_solana ;;
  mina) cmd_mina ;;
  blockscout) cmd_blockscout ;;
  ...
  faucet) cmd_faucet ;;
  toon-client) cmd_toon_client ;;
  probe-toon-client) cmd_probe_toon_client ;;
  all) cmd_all ;;
...
Commands:
  ...
  townhouse          Deploy townhouse.sdl.yaml — full operator stack (apex
                     ...
                     across redeploys.
  probe-toon-client  Probe the toon-client /healthz from leases.json

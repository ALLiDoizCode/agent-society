/**
 * CLI tests for `townhouse wallet show` (Phase 3 cards layout + flags) and
 * `townhouse wallet seed` (epic-49, Phase 3).
 *
...
function makeTempDir(): string {
  const dir = join(
    tmpdir(),
    `townhouse-cli-phase3-${randomBytes(8).toString('hex')}`
  );
...
describe('wallet show (Phase 3 cards + flags)', () => {
  ...
  it('default cards layout shows npub for all three nodes, no hex', async () => {
    ...
    try {
      ...
      expect(output).toContain('townhouse wallet show --json');
      expect(output).toContain('townhouse wallet show --hex');
      expect(output).toContain('townhouse wallet show --paths');
      expect(output).toContain('townhouse credits buy');
    } finally {
...
describe('wallet seed (Phase 3)', () => {
  ...
  it('with --confirm + valid password: prints warning banner + mnemonic', async () => {
    ...
    try {
      ...
      expect(out).toMatch(/townhouse init/);

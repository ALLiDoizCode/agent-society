 ...
 * `townhouse hs up` apex + connector + a provisioned town peer using
 ...
 *   AC #5: townhouse status --units=sats --rate 1500 renders sats live.
 *
...
// Mirrors townhouse-earnings-e2e.test.ts:145-180. Not extracted to _test-helpers
  ...
  if (bound.length > 0) {
    throw new Error(
      `Cannot start HS apex: ports already bound: ${bound.join(', ')}. ` +
        ...
        `(docker rm -f townhouse-hs-* or scripts/townhouse-dev-infra.sh down) ` +
        `and re-run the gate.`
    );
  }
}
...
describe.skipIf(!shouldRun)(
  'townhouse TUI E2E gate — real CLI against real Docker (Story 48.7)',
  () => {
    let tmpDir: string;
    ...
    beforeAll(async () => {
      ...
      tmpDir = mkdtempSync(join(tmpdir(), 'townhouse-tui-e2e-'));
      ...
      // 2. townhouse hs up — cold-boot; exits 0 after apex is published.
      const up = runCli('hs', {
        configDir: tmpDir,
        password: TEST_PASSWORD,
        env: { TOWNHOUSE_WALLET_PASSWORD: TEST_PASSWORD },
      ...
      // 4. Wait for townhouse-api /api/transport — NOT /health (46.4 finding).

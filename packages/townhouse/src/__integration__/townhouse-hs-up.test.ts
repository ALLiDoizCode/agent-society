...
function cleanupContainersAndVolumes(): void {
  ...
  for (const vol of [HS_ANON_VOLUME, 'townhouse-hs-town-data', 'townhouse-hs-mill-data', 'townhouse-hs-dvm-data']) {
    try {
...
describe.skipIf(!shouldRun)(
  'townhouse hs up — real CLI apex boot',
  () => {
    let tmpDir: string;
    ...
    it('idempotent re-run: same hostname, no new containers', async () => {
      ...
      const firstRun = JSON.parse(readFileSync(join(tmpDir, 'host.json'), 'utf-8')) as { hostname: string };
    ...
    it('re-up after down produces same hostname (volume preserved → same keypair)', async () => {
      const prevHostname = (JSON.parse(
        readFileSync(join(tmpDir, 'host.json'), 'utf-8')
      ) as { hostname: string }).hostname;
      ...
      const newHostJson = JSON.parse(readFileSync(join(tmpDir, 'host.json'), 'utf-8')) as { hostname: string };

 ...
 * `townhouse hs up` apex + a real connector + a provisioned town peer:
 *
 ...
 *     — conflict with townhouse-dev-infra.sh; do not run both stacks at once
 *
...
if (!shouldRun) {
  console.warn(
    '\n⚠️  Skipping earnings data plane E2E gate (Story 47.5).\n' +
      ...
      '   Ensure ports 9401 (connector admin) and 28090 (townhouse-api) are free.\n'
  );
}
...
// 46.4 Finding 'test endpoint': townhouse-api does NOT serve `/health`.
...
// file to avoid a shared mutation surface). Mirrors townhouse-node-lifecycle-
// e2e.test.ts:74-111 and townhouse-hs-up.test.ts:54-91.

// Code-review P8: anchor the filter to exact HS-container names instead of the
// substring `townhouse-hs-`, so concurrent test runs (or unrelated containers
  ...
  if (bound.length > 0) {
    throw new Error(
      `Cannot start HS apex: ports already bound: ${bound.join(', ')}. ` +
        `Stop any concurrent townhouse stack ` +
        `(scripts/townhouse-dev-infra.sh down, docker rm -f townhouse-hs-* townhouse-*) ` +
        `and re-run the gate.`
    );
  }
}
...
describe.skipIf(!shouldRun)(
  'townhouse earnings data plane E2E gate — real CLI against real Docker (Story 47.5)',
  () => {
    let tmpDir: string;
    ...
    beforeAll(async () => {
      ...
      tmpDir = mkdtempSync(join(tmpdir(), 'townhouse-earnings-e2e-'));
      ...
      // which is bind-mounted at `/.townhouse/earnings-snapshots.jsonl` inside
      // the townhouse-api container (compose line: `${TOWNHOUSE_HOME}:/.townhouse:rw`).
      ...
      // 1. townhouse init — writes config.yaml, wallet, compose templates.
      ...
      // 2. townhouse hs up — cold-boot path; exits 0 after apex is published.
      ...
      // 4. Wait for townhouse-api /api/transport to return 200.
      //    CRITICAL: use /api/transport, NOT /health — 46.4 Finding 'test
      //    endpoint' confirmed the townhouse-api image does NOT serve /health.
      ...
      // DEFAULT_HS_CHAIN_PROVIDERS by default, so `townhouse hs up` ships a
      ...
      if (!/^chainProviders\s*:/m.test(generatedYaml)) {
        throw new Error(
          'Epic 47 BUG-1 regression: generated connector.yaml has no ' +
            ...
            'DEFAULT_HS_CHAIN_PROVIDERS. Check `packages/townhouse/src/config/' +
      ...
      if (addCode !== 0) {
        throw new Error(
          `townhouse node add town exited ${addCode}. stdout: ${addStdout}`
        );
      }
      ...
      const addBody = parseLastJsonLine<{
        ...
        type: string;
        peerId: string;
      }>(addStdout, 'townhouse node add town');
      ...
      //    2s interval (mirror townhouse-node-lifecycle-e2e.test.ts:257-279).
      ...
      //    container on the townhouse-hs-net network. Both are deferred to
    ...
    afterAll(async () => {
      ...
      if (tmpDir) {
        try {
          ...
          await waitForExitLabelled(down.process, 60_000, 'townhouse hs down');
        } catch {
          /* best-effort */
        }
      }

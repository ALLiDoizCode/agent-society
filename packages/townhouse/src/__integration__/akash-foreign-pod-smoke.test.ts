 ...
 *   - townhouse hs up (cold-boot):                 ~5 min
 ...
 * TWO concurrent `townhouse hs up` stacks on the same host, which 49.1
...
describe.skipIf(!shouldRun)(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      // g.townhouse.town. `townhouse hs up` boots the apex only (connector +
      ...
      // TOWNHOUSE_WALLET_DIR must match hs up so the api isn't recreated.
      {
        ...
        if (existsSync(townComposePath)) {
          ...
          // This replicates what `townhouse node add town` does via POST /api/nodes
          // (which is not available in the production townhouse-api image).
          await adminClientA.registerPeer({
            id: 'townhouse-hs-town-smoke',
            url: 'ws://townhouse-hs-town:3000/btp',
            authToken: '',

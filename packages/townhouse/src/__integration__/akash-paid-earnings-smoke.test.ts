...
describe.skipIf(!shouldRun)(
  ...
  () => {
    ...
    beforeAll(async () => {
        ...
        if (/^chainProviders:/m.test(existing)) {
          throw new Error(
            `[49.4] config.yaml already contains a chainProviders: key — refusing to append duplicate. ` +
              `Either townhouse init now emits chainProviders natively (update this test to patch in place), ` +
      ...
      // Ref: townhouse-hs-connector.yaml comment "town → acct[4] = 0x15d34AAf..."
      //

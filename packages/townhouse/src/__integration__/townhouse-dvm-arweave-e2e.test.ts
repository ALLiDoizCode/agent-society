...
const HS_CONTAINER_NAMES = [HS_CONNECTOR_NAME, 'townhouse-hs-api', 'townhouse-hs-town'] as const;
...
const B_PRIVATE_KEY = '0x47e179ec197488593b187f80a00eb0da91f1b9d0b13f8733639f19c30a34926a';
...
function loadImageFromManifest(key: 'connector' | 'dvm' | 'town'): string {
  const p = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'dist', 'image-manifest.json');
  if (!existsSync(p)) throw new Error(`dist/image-manifest.json missing — run CI download`);
...
function probePortFree(port: number): Promise<boolean> {
  return new Promise<boolean>((resolve) => {
    ...
    try { s = createConnection({ port, host: '127.0.0.1' }); } catch { resolve(false); return; }
    let done = false;
    const settle = (free: boolean) => { if (done) return; done = true; s.destroy(); resolve(free); };
    s.once('connect', () => settle(false));
    s.once('error', (e: NodeJS.ErrnoException) => settle(e.code === 'ECONNREFUSED'));
    s.setTimeout(1_000, () => settle(false));
  });
}

async function assertPortsFree(): Promise<void> {
  const ports = [9401, 28090, 9402, 9050, B_BTP_PORT, B_HEALTH_PORT, DVM_BLS_PORT];
  const bound = (
    await Promise.all(ports.map((p) => probePortFree(p).then((free) => ({ p, free }))))
  ).filter((c) => !c.free).map((c) => c.p);
  if (bound.length) throw new Error(`Ports already bound: ${bound.join(', ')} — stop any running stack`);
}

async function waitForExitLabelled(child: ChildProcess, ms: number, label: string): Promise<number> {
  const code = await waitForExit(child, ms).catch((e) => {
    throw new Error(`[${label}] timeout (${ms}ms): ${e instanceof Error ? e.message : e}`);
  });
...
async function startConnectorContainer(
  ...
  try { execSync(`docker rm -f ${containerName}`, { stdio: 'pipe', timeout: 30_000 }); } catch { /* ok */ }
  try { execSync(`docker volume rm -f ${volumeName}`, { stdio: 'pipe', timeout: 30_000 }); } catch { /* ok */ }
  execSync(`docker volume create ${volumeName}`, { stdio: 'pipe', timeout: 30_000 });
  execSync(
    `docker run --rm --platform linux/amd64 -v ${volumeName}:/data busybox sh -c "chown -R 1000:1000 /data && chmod 700 /data"`,
    { stdio: 'pipe', timeout: 60_000 }
  );
  writeFileSync(join(configDir, 'connector.yaml'), configYaml, { encoding: 'utf-8', mode: 0o644 });
  execSync(
    `docker run -d \
      --name ${containerName} \
  ...
  if (state !== 'running') {
    const logs = execSync(`docker logs --tail 30 ${containerName} 2>&1`, { encoding: 'utf-8', timeout: 5_000 });
    throw new Error(`${containerName} state=${state} (expected running). Logs:\n${logs.trim()}`);
  }
}
...
async function waitForSocks5(timeoutMs = 240_000): Promise<void> {
  ...
  while (Date.now() < deadline) {
    const ok = await new Promise<boolean>((res) => {
      const s = createConnection({ host: '127.0.0.1', port: 9050 }, () => { s.destroy(); res(true); });
      s.once('error', () => res(false));
      s.setTimeout(2_000, () => { s.destroy(); res(false); });
    });
    if (ok) return;
    await sleep(3_000);
  }
  throw new Error(`B anon SOCKS5 (127.0.0.1:9050) not ready within ${timeoutMs}ms`);
}
...
describe.skipIf(!shouldRun)(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      // ── CLI binary ──────────────────────────────────────────────────────────
      const cliBin = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'dist', 'cli.js');
      ...
      const bYaml = [
        ...
        `btpServerPort: ${B_BTP_PORT}`, `healthCheckPort: ${B_HEALTH_PORT}`,
        'environment: development', 'deploymentMode: standalone', 'logLevel: warn',
        'adminApi:', '  enabled: true', '  port: 9402', '  host: 127.0.0.1',
        "  allowedIPs: ['127.0.0.1/32']",
        'transport:', '  type: socks5', '  socksProxy: socks5h://127.0.0.1:9050',
        '  managed: true', '  externalUrl: auto', '  managedOptions:',
        '    hiddenServiceDir: /var/lib/anon/hs', `    hiddenServicePort: ${B_BTP_PORT}`,
        '    startupTimeoutMs: 360000',
        'chainProviders: []', 'peers: []', 'routes: []',
      ].join('\n');
      console.log('[49-5] Starting B connector...');
      await startConnectorContainer(B_CONNECTOR_NAME, B_ANON_VOLUME, bYaml, bConfigDir, connectorImage);
      ...
      const initCode = await waitForExitLabelled(
        runCli('init', { configDir: tmpDirA, password: TEST_PASSWORD, env: { TOWNHOUSE_WALLET_PASSWORD: TEST_PASSWORD } }).process,
        30_000, 'townhouse init A'
      );
      if (initCode !== 0) throw new Error(`townhouse init exited ${initCode}`);

      const upCode = await waitForExitLabelled(
        runCli('hs', { configDir: tmpDirA, password: TEST_PASSWORD, env: { TOWNHOUSE_WALLET_PASSWORD: TEST_PASSWORD }, extraArgs: ['up'] }).process,
        360_000, 'townhouse hs up A'
      );
      if (upCode !== 0) throw new Error(`townhouse hs up exited ${upCode}`);

      // ── Capture A's hostname ────────────────────────────────────────────────
      const hostJson = JSON.parse(readFileSync(join(tmpDirA, 'host.json'), 'utf-8')) as { hostname: string };
      ...
      try {
        const hsGwOut = execSync(
          ...
          { encoding: 'utf-8', timeout: 5_000 }
        ).trim().split('\n').find((s) => /^\d+\.\d+\.\d+\.\d+$/.test(s.trim()))?.trim();
      ...
      try {
        ...
        if (connectorBridgeIp && /^\d+\.\d+\.\d+\.\d+$/.test(connectorBridgeIp)) {
          dvmBtpConnectorUrl = `ws://${connectorBridgeIp}:3000`;
          console.log(`[49-5] Connector bridge IP: ${connectorBridgeIp} → DVM BTP: ${dvmBtpConnectorUrl}`);
        }
      } catch (e) {
        console.warn(`[49-5] Could not get connector bridge IP: ${(e as Error).message}`);
      }
      ...
      console.log('[49-5] Starting town relay (townhouse-hs-net, CONNECTOR_URL=ws://connector:3000)...');
      ...
      try {
        ...
        if (townState !== 'running') {
          const logs = execSync('docker logs --tail 20 townhouse-hs-town 2>&1', { encoding: 'utf-8', timeout: 5_000 });
          console.warn(`[49-5] town relay state=${townState}. Logs:\n${logs.trim()}`);
          aDestination = 'g.townhouse';
        } else {
          console.log('[49-5] Town relay started on townhouse-hs-net (CONNECTOR_URL=ws://connector:3000)');
        }
      } catch (e) {
        console.warn(`[49-5] docker run town failed: ${(e as Error).message} — falling back to g.townhouse`);
        aDestination = 'g.townhouse';
      }
      ...
      if (aDestination !== 'g.townhouse') {
        ...
        try {
          ...
          console.log(`[49-5] Town relay registered (${relayBtpUrl}), destination: g.townhouse.town`);
        } catch (e) {
          console.warn(`[49-5] registerPeer failed: ${(e as Error).message} — falling back to g.townhouse`);
          aDestination = 'g.townhouse';
        }
        ...
        if (aDestination === 'g.townhouse.town') {
          console.log('[49-5] Waiting for town relay inbound BTP connection...');
          const townDeadline = Date.now() + 30_000;
          while (Date.now() < townDeadline) {
            const peers = await adminClientA.getPeers().catch(() => []);
            const townPeer = peers.find((p) => (p.id as string)?.includes('town'));
      ...
      const clientConfig = {
        ...
        ilpInfo: { pubkey: bPubkey, ilpAddress: bIlpAddress, btpEndpoint: `ws://${hostnameA}:3000/btp`, assetCode: 'USD', assetScale: 6 },
      ...
      console.log('[49-5] Starting ToonClient (.anyone BTP, up to 3 retries)...');
      let startOk = false;
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          console.warn(`[49-5] start() attempt ${attempt} failed: ${(e as Error).message}`);
          try { await toonClient?.stop(); } catch { /* ok */ }
        }
      }
      if (!startOk) throw new Error('ToonClient.start() failed after 3 attempts');
      ...
      const negotiations = (toonClient as any).peerNegotiations as Map<string, unknown>;
      if (negotiations instanceof Map) {
        negotiations.set('town', { chain: CHAIN_KEY, chainType: 'evm', chainId: CHAIN_ID, settlementAddress: A_EVM_ADDR, tokenAddress: TOKEN_ADDR, tokenNetwork: TOKEN_NETWORK });
      }
      try {
      ...
      const ev1: NostrEvent = finalizeEvent(
        { kind: 1, content: `49-5 smoke @ ${new Date().toISOString()}`, tags: [['t', '49-5-smoke']], created_at: Math.floor(Date.now() / 1000) },
        bSecretKey
      );
      kind1EventId = ev1.id;
      ...
      console.log(`[49-5] kind:5094 destination: ${dvmDestination} (via localDelivery → DVM port 3300)`);
      ...
      console.log(`[49-5] Publishing kind:5094 (${blob.length}B) to ${dvmDestination}...`);
      try {
        // Use ilpAmount=0n so the DVM processes for free (unauthenticated Turbo path)
        dvmResult = await toonClient.publishEvent(dvmEv, { ilpAmount: 0n, destination: dvmDestination });
      } catch (e) {
        dvmResult = { success: false, error: (e as Error).message };
      }
      console.log(`[49-5] kind:5094: success=${dvmResult.success}, data=${dvmResult.data?.slice(0, 20) ?? 'none'}`);
    }, 1_200_000);
        ...
        if (tmpDirA) {
          try {
            await waitForExitLabelled(
              runCli('hs', { configDir: tmpDirA, password: TEST_PASSWORD, env: { TOWNHOUSE_WALLET_PASSWORD: TEST_PASSWORD }, extraArgs: ['down'] }).process,
              60_000, 'hs down'
            );
          } catch (e) { console.warn(`[49-5 afterAll] hs down: ${(e as Error).message}`); }
        }
        cleanupAll();
        try {
          const orphans = execSync(`docker ps -aq --filter "name=townhouse-"`, { encoding: 'utf-8', timeout: 10_000 }).trim();
          if (orphans) execSync(`docker rm -f ${orphans.split('\n').join(' ')}`, { stdio: 'pipe', timeout: 30_000 });
        } catch { /* best-effort */ }
        if (tmpDirA) rmSync(tmpDirA, { recursive: true, force: true });
        if (bConfigDir) rmSync(bConfigDir, { recursive: true, force: true });
      } finally {
        if (priorPwd === undefined) delete process.env['TOWNHOUSE_WALLET_PASSWORD'];
    ...
    it('kind:1 published via .anyone HS; transport invariants hold (AC #1 + #3)', () => {
      expect(kind1Result.success, `AC #1 FAIL: ${(kind1Result as { error?: string }).error ?? 'unknown'}`).toBe(true);
      // kind1EventId was set from ev1.id before publish — verify it's a real event id
      expect(kind1EventId).toMatch(/^[0-9a-f]{64}$/);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const cfg = (toonClient as any)['config'] as { transport?: { type: string; socksProxy?: string }; btpUrl?: string };
    ...
    it('kind:5094 DVM returns Arweave txid in ILP FULFILL data (AC #2)', () => {
      expect(dvmResult.success, `AC #2 FAIL: ${(dvmResult as { error?: string }).error ?? 'unknown'}. DVM may need Turbo unauthenticated access.`).toBe(true);
      expect(dvmResult.data, 'AC #2: FULFILL data must be present').toBeTruthy();
    ...
    it('connector healthy and B BTP session active — earnings confirmed (AC #4)', async () => {
      // Connector health
      const health = await fetch(CONNECTOR_HEALTH_URL, { signal: AbortSignal.timeout(10_000) });
      expect(health.ok, `AC #4: connector health ${CONNECTOR_HEALTH_URL} returned ${health.status}`).toBe(true);
      ...
      const channels = await fetch(`${CONNECTOR_ADMIN_URL}/admin/channels`, { signal: AbortSignal.timeout(10_000) });
      expect(channels.ok, `AC #4: /admin/channels returned ${channels.status}`).toBe(true);
    ...
    it('chain endpoints are Akash-hosted (not 127.0.0.1) (AC #5)', async () => {
      ...
      expect(evmRes.ok, `AC #5: Anvil ${evmUrl} HTTP ${evmRes.status}`).toBe(true);
      const evmBody = (await evmRes.json()) as { result?: unknown };
      expect(typeof evmBody.result === 'string' && evmBody.result.startsWith('0x')).toBe(true);
      ...
      expect(solRes.ok, `AC #5: Solana ${solUrl} HTTP ${solRes.status}`).toBe(true);

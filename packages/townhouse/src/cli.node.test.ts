...
function makeConnRefused(extra?: Partial<CliNodeCommandOverrides>): CliNodeCommandOverrides {
  return {
    apiUrl: ADD_URL,
    fetch: vi.fn().mockRejectedValue(new Error('connect ECONNREFUSED 127.0.0.1:28090')),
    ...extra,
  };
}
...
describe('CLI node subcommand', () => {
  ...
  it('node add with no type → POSTs {type: "town"} (AC #3)', async () => {
    const overrides = makeNodeOverrides(201, {
      id: 'town', type: 'town', peerId: 'town',
      ilpAddress: 'g.townhouse.town', hsRoute: 'g.townhouse.town',
  ...
  it('node add mill → POSTs {type: "mill"} and prints four-stage progress', async () => {
    const overrides = makeNodeOverrides(201, {
      id: 'mill', type: 'mill', peerId: 'mill',
      ilpAddress: 'g.townhouse.mill', hsRoute: 'g.townhouse.mill',
      healthCheckUrl: 'http://townhouse-hs-mill:3200/health',
    });
    await main(['node', 'add', 'mill'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBeUndefined();
    const fetched = (overrides.fetch as ReturnType<typeof vi.fn>).mock.calls[0];
    const body = JSON.parse(fetched[1].body as string);
  ...
  it('node add on 201 — stdout includes all four stages and the new node id', async () => {
    ...
    await main(['node', 'add', 'town'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBeUndefined();
    const out = stdoutText();
    expect(out).toContain('Pulling image');
    expect(out).toContain('Deriving wallet');
  ...
  it('node add on 502 step:pull-image → exit 1, renderFailure called', async () => {
    ...
    const err = (renderFailure as ReturnType<typeof vi.fn>).mock.calls[0][0] as Error;
  ...
  it('node add on 409 node_type_in_use → exit 1, message includes existingId', async () => {
    const overrides = makeNodeOverrides(409, {
      error: 'node_type_in_use', type: 'town', existingId: 'town',
    });
    await main(['node', 'add', 'town'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBe(1);
  ...
  it('node add --json on 201 → stdout is single-line valid JSON with ok:true, no human prose', async () => {
    ...
    await main(['node', 'add', '--json'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBeUndefined();
  ...
  it('node add --help → stdout contains upsell string (AC #7)', async () => {
    await expect(
      main(['node', 'add', '--help'], undefined, undefined, undefined, undefined)
    ).rejects.toThrow(CliHelpRequested);
    const out = consoleText();
    expect(out).toContain('townhouse node add mill   # earn from chain swaps (5x earnings unlock)');
  });
  ...
  it('node remove <id> with TTY and no --yes → prompts; cancels on "n"', async () => {
    ...
    await main(['node', 'remove', 'town'], undefined, undefined, undefined, overrides);
  ...
  it('node remove <id> with TTY and --yes → does NOT prompt; DELETEs immediately', async () => {
    ...
    await main(['node', 'remove', 'town', '--yes'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBeUndefined();
  ...
  it('node remove with no id → exit 1 with usage', async () => {
    const overrides: CliNodeCommandOverrides = { apiUrl: ADD_URL, fetch: vi.fn() };
  ...
  it('node remove <id> on 404 → exit 1 with "No node with id" message', async () => {
    const overrides = makeNodeOverrides(404, { error: 'unknown_node', id: 'town' });
    await main(['node', 'remove', 'town', '--yes'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBe(1);
  ...
  it('node remove invalid_ID (uppercase) → exit 1, regex rejection (no HTTP call)', async () => {
    const overrides: CliNodeCommandOverrides = { apiUrl: ADD_URL, fetch: vi.fn() };
    await main(['node', 'remove', 'Town-01', '--yes'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBe(1);
    expect((overrides.fetch as ReturnType<typeof vi.fn>)).not.toHaveBeenCalled();
    const err = stderrText();
  ...
  it('node remove <id> --json on 200 → stdout is {"ok":true, id, type}', async () => {
    ...
    await main(['node', 'remove', 'town', '--yes', '--json'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBeUndefined();
    const lines = stdoutText().trim().split('\n');
    expect(lines).toHaveLength(1);
    const parsed = JSON.parse(lines[0]!);
  ...
  it('node list with 2 nodes → 4-column table with correct status values', async () => {
    const overrides = makeNodeOverrides(200, {
      nodes: [
        { id: 'town', type: 'town', peerId: 'town', ilpAddress: 'g.townhouse.town', status: 'connected', enabledAt: '2026-05-11T00:00:00.000Z', lastSeenAt: null },
        { id: 'mill', type: 'mill', peerId: 'mill', ilpAddress: 'g.townhouse.mill', status: 'disconnected', enabledAt: '2026-05-11T00:00:00.000Z', lastSeenAt: null },
      ],
    });
    await main(['node', 'list'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBeUndefined();
  ...
  it('node list --json → emits the body verbatim (no ok envelope)', async () => {
    ...
    await main(['node', 'list', '--json'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBeUndefined();
    const lines = stdoutText().trim().split('\n');
    expect(lines).toHaveLength(1);
    const parsed = JSON.parse(lines[0]!);
  ...
  it('node list with connector down → status:"unknown" rendered in table; exit 0', async () => {
    const overrides = makeNodeOverrides(200, {
      nodes: [
        { id: 'town', type: 'town', peerId: 'town', ilpAddress: 'g.townhouse.town', status: 'unknown', enabledAt: '2026-05-11T00:00:00.000Z', lastSeenAt: null },
      ],
    });
    await main(['node', 'list'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBeUndefined();
  ...
  it('node unknown-verb → exit 1, prints usage', async () => {
    await main(['node', 'frobnicate'], undefined, undefined, undefined, undefined);
    expect(process.exitCode).toBe(1);
  ...
  it('node add foo → exit 1 with "Unknown type" stderr, no HTTP call', async () => {
    const overrides: CliNodeCommandOverrides = { apiUrl: ADD_URL, fetch: vi.fn() };
    await main(['node', 'add', 'foo'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBe(1);
    expect((overrides.fetch as ReturnType<typeof vi.fn>)).not.toHaveBeenCalled();
  ...
  it('node add on 409 node_lifecycle_in_flight → exit 1 with "in flight" message', async () => {
    const overrides = makeNodeOverrides(409, { error: 'node_lifecycle_in_flight' });
    await main(['node', 'add', 'town'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBe(1);
  ...
  it('node add timeout --json → emits ok:false, error:"timeout" JSON', async () => {
    ...
    await main(['node', 'add', 'town', '--json'], undefined, undefined, undefined, overrides);
    expect(process.exitCode).toBe(1);
  ...
  it('node remove --help → NODE_REMOVE_HELP shown (not generic NODE_HELP)', async () => {
    await expect(
      main(['node', 'remove', '--help'], undefined, undefined, undefined, undefined)
    ).rejects.toThrow(CliHelpRequested);
    const out = consoleText();
  ...
  it('node list --help → NODE_LIST_HELP shown (not generic NODE_HELP)', async () => {
    await expect(
      main(['node', 'list', '--help'], undefined, undefined, undefined, undefined)
    ).rejects.toThrow(CliHelpRequested);
    const out = consoleText();

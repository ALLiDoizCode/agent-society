/**
 * CLI tests for `townhouse credits buy` / `townhouse credits balance`
 * (epic-49, Phase 2).
 *
...
function makeTempDir(): string {
  const dir = join(
    tmpdir(),
    `townhouse-credits-cli-test-${randomBytes(8).toString('hex')}`
  );
    ...
    it('requires --token + --amount', async () => {
      ...
      try {
        ...
        const err = consoleErrorSpy.mock.calls
          .map((c) => String(c[0]))
          .join('\n');
        expect(err).toMatch(/Usage: townhouse credits buy/);
    ...
    it('requires --token', async () => {
      ...
      try {
        ...
        const err = consoleErrorSpy.mock.calls
          .map((c) => String(c[0]))
          .join('\n');
        expect(err).toMatch(/Usage: townhouse credits balance/);
        expect(process.exitCode).toBe(1);
      } finally {
        rmSync(dir, { recursive: true, force: true });
      }
    ...
    it('prints usage and exits 1', async () => {
      ...
      try {
        ...
        const err = consoleErrorSpy.mock.calls
          .map((c) => String(c[0]))
          .join('\n');
        expect(err).toMatch(/townhouse credits buy/);
        expect(err).toMatch(/townhouse credits balance/);
        expect(process.exitCode).toBe(1);
      } finally {
        rmSync(dir, { recursive: true, force: true });
      }

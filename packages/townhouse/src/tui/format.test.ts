...
describe('formatRelativeTime', () => {
  ...
  it('returns ? for non-ISO input', () => {
    expect(formatRelativeTime('not-an-iso', new Date('2026-05-14T12:00:00Z'))).toBe('?');
  });

  it('returns <1m ago for delta < 60s', () => {
    expect(formatRelativeTime('2026-05-14T11:59:30Z', new Date('2026-05-14T12:00:00Z'))).toBe('<1m ago');
  });

  it('returns Nm ago for delta in minutes', () => {
    expect(formatRelativeTime('2026-05-14T11:55:00Z', new Date('2026-05-14T12:00:00Z'))).toBe('5m ago');
  });

  it('returns Nh ago for delta in hours', () => {
    expect(formatRelativeTime('2026-05-14T10:00:00Z', new Date('2026-05-14T12:00:00Z'))).toBe('2h ago');
  });

  it('returns Nd ago for delta in days', () => {
    expect(formatRelativeTime('2026-05-12T12:00:00Z', new Date('2026-05-14T12:00:00Z'))).toBe('2d ago');
  });

  it('returns Nmo ago for delta in months', () => {
    expect(formatRelativeTime('2026-02-14T12:00:00Z', new Date('2026-05-14T12:00:00Z'))).toBe('2mo ago');
  });

  it('truncates (floor) rather than rounds minutes', () => {
    // 89s → Math.floor(89/60) = 1m, not 2m
    expect(formatRelativeTime('2026-05-14T11:58:31Z', new Date('2026-05-14T12:00:00Z'))).toBe('1m ago');
  });

  it('returns <1m ago for future ISO (negative delta treated as <1m)', () => {
    expect(formatRelativeTime('2026-05-14T12:01:00Z', new Date('2026-05-14T12:00:00Z'))).toBe('<1m ago');
  });

  it('handles real-world aggregator ISO with ms precision', () => {
    expect(formatRelativeTime('2026-05-13T18:42:11.123Z', new Date('2026-05-14T06:42:11.123Z'))).toBe('12h ago');
  });
});

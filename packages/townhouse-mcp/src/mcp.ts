#!/usr/bin/env node
/**
 * `townhouse-mcp` — a thin MCP stdio server exposing the Townhouse OPERATOR
 * surface to a Claude agent (Desktop or Code). It holds NO chain keys: every
 * tool maps to either the `townhouse` CLI (lifecycle / config / $) or the apex
 ...
 *   • Claude Code   — `claude mcp add townhouse -- townhouse-mcp`.
 */

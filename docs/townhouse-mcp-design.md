...
- **P1b — containerized `townhouse-api` (REMAINING).** The apex `up`/`hs up` boots a _separate_ `ghcr.io/toon-protocol/townhouse-api` image whose own entrypoint enforces the password at startup. It serves `:9400` — the surface the MCP server's API-backed tools (`balances`, `earnings`, `list_nodes`, `withdraw`) use. So mnemonic-mode apex needs: (1) `TOWNHOUSE_MNEMONIC: '${TOWNHOUSE_MNEMONIC:-}'` passthrough in both compose templates, and (2) the container entrypoint taught the same direct-load path, then a rebuilt image.
...
- **P1b — containerized `townhouse-api` mnemonic support (§3). REMAINING.** Compose passthrough + the `townhouse-api` image entrypoint's direct-load path + rebuilt image. Required for the apex `:9400` API (the MCP server's `balances`/`earnings`/`list_nodes`/`withdraw` tools).
